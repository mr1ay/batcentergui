...
echo regulator time
		...
		for /l %%e in (0,1,10) do   (
			if exist _parse%%e.mr1ay del _parse%%e.mr1ay 
		...
		for /f "tokens=*" %%u in ( _batcenterpluginlist.mr1ay ) do (
			set /a a=!a!+1
			if !a! leq  21   set saver=_parse1.mr1ay
			if !a! geq  21   if !a! leq 41  set saver=_parse2.mr1ay
			if !a! geq  41   if !a! leq 61  set saver=_parse3.mr1ay
			if !a! geq  61   if !a! leq 81  set saver=_parse4.mr1ay
			if !a! geq  81   if !a! leq 101 set saver=_parse5.mr1ay
			if !a! geq 101   if !a! leq 121 set saver=_parse6.mr1ay
			if !a! geq 121   if !a! leq 141 set saver=_parse7.mr1ay
			if !a! geq 141   if !a! leq 161 set saver=_parse8.mr1ay
			if !a! geq 161   if !a! leq 181 set saver=_parse9.mr1ay
			if !a! geq 181   if !a! leq 201 set saver=_parse10.mr1ay
			echo.%%u >>!saver!
			)	 
		...
		for /l %%e in (0,1,10) do   if exist _parse%%e.mr1ay (
			echo.%%e>_page_count.mr1ay
			for /f "usebackq" %%u in ("_parse%%e.mr1ay") do set /a count=count+1 
			echo.set countline%%e= !count! >>_allcountline.bat &&set count=0
			)
	...
	set /p countpage=<_page_count.mr1ay
	...
	:loop
		...
		echo.        >>%realpage%
		echo.        type  _parse%countloop%.mr1ay>>%realpage%
		echo.        >>%realpage% 
	...
	for /l %%e in (1,1,%countpage%) do (
		...
		:loop3
			...
			echo.               if %%errorlevel%% equ %limitline%  echo.!countloop3!^>_selectpage.mr1ay ^&^& call page!countloop3!.bat>>%realpage%
...
::pause
			...
			::if %errorlevel% == 25 echo.1>_selectpage.mr1ay  && call batgui :startup
			::if %errorlevel% == 26 echo.2>_selectpage.mr1ay  && call batgui :startup
